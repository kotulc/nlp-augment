...
def get_metric_record(session: Session, name: str, section_id: UUID=None) -> Optional[Metric]:
...
def add_metric(session: Session, name: str, value: float, section_id: UUID=None) -> Metric:
    ...
    """
    existing = get_metric_record(session, section_id, name)
    if existing:
...
def delete_metric(session: Session, name: str, section_id: UUID=None) -> bool:
...
def handle_metrics_request(session: Session, request: MetricsRequest, configs: dict) -> List[Metric]:
    ...
    for name, value in results.items():
        metric = add_metric(session, name, value, section_id=request.section_id)
